...
#                for j in range(columns):
#                    result=0
#                    for k in range(len(X_t[0])):
#                        result+=X[i][j]*X_t[j][k]
#                    X_mul[i][k]=result
##                    print(result)
##            print('return xmul')
#            return X_mul
...
#        print(len(X_t),',',len(X_t[0]))
#        X_t_Y=self.array_mul(X_t,Y)
        print('inv shape {}*{}'.format(len(inv),len(inv[0])))
#        print('X_t*Y is : \n'.format(X_t_Y))
#        print('X_t_Y shape {}*{}'.format(len(X_t_Y),len(X_t_Y[0])))
        
        W_1=self.array_mul(inv,X_t)
        
#        W=self.array_mul(self.array_mul(inv,X_t),Y)
#        W=self.arrary_add(inv,self.array_mul(X_t,Y),'-')
#        
...
#m=[[2 for i in range(5)] for j in range(1)]
#n=[[3 for j in range(1)] for i in range(5)]
#reg=Regression(5,40)
#r=reg.array_mul(m,n)
#print(r)
